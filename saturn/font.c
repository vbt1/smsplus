...
Sint32	i=0,j;
Uint8	*codep;
unsigned int col[4]={2,1,2,3};//{0,1,2,3};
...
//	{
	    for(i=0;i<0x10;i++) 
		{
			for(j=3;j>-1;j-=2) 
			{
				vram[wp]= (Uint8 )((col[ (codep[i]>>(j<<1) ) & (0x03)]<<4) |col[(codep[i]>>((j-1)<<1)) & (0x03)]);
                wp++;
			}
			if(i%2)
			       wp += (252);//(FntXsize-4);
...
Uint16	wx,wy;
	...
	while(str[i]) 
	{
			FNT_Print1Char256_8x8_2bpp(vram,str[i],wx,wy);
			wx += 4;
		i++;
	}
}
