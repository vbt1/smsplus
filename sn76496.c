...
/*
VBT : Mapper le volume de la sms avec celui de la saturn 
0-7 vers 0-127

mapper la frequence sms vers le pich de la saturn
0-1023 vers ....0-8191 por du 8khz

freq = ( clock / period )
clock=3579545 (master_clock)

Farie un square wave :

0x000 puis 0x7FFF en boucle 
ou 0x000,0x000 puis 0x7FFF,0x7FFF
ou plus de repetitions
...
void SN76496Write(int data)
{
	...
	{
		unsigned int r = sn.LastRegister;
		unsigned int c = r>>1;
...
//			if (sn.Count[i] <= length*STEP) sn.Count[i] += length*STEP;
			if (sn.Count[i] <= (length<<16)) sn.Count[i] += (length<<16);
		}
	}
...
//		if (out > MAX_OUTPUT * STEP) out = MAX_OUTPUT * STEP;
		if (out > (MAX_OUTPUT <<16)) out = MAX_OUTPUT <<16;
...
static void SN76496_set_clock(unsigned int clock)
{
...
int SN76496_init(unsigned int clock,unsigned int sample_rate)
{
	int i;
	//char name[40];
